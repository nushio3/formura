...
extensionNSet = unsafePerformIO $ do
  argv <- getArgs
  return $ if "--extend" `elem` argv then [1..8] else [7]
...
benchmark it = do
  ...
  withCurrentDirectory exeDir $ do
    writeFile "submit.sh" $ unlines
      ...
      , ""
      , unlines [ printf "fapp -C -d prof-X%d -Hevent=Statistics  mpiexec -n %d   -ofout-proc out/out -oferr-proc out/err  ./a.out %d %d" n mpiSize x (8192::Int)
