...
program main
  ...
  double precision, save, dimension(NX,NY,NZ) :: u,v
  double precision, save, dimension(NX,NY,NZ) :: u_other,v_other
  integer t
  call init()
  ...
  print *, u(NX/2,NY/2,NZ/2), v(NX/2,NY/2,NZ/2)
...
contains
  ...
  subroutine proceed(u,v,u_next,v_next)
    ...
    !$omp do collapse(2) private(z0,y0,x0,x,y,z,u0, v0, eat, du_dt, dv_dt, lap_u, lap_v)
    do z0 = 1, (NZ-2)/2
       do y0 = 1, NY-2
          do x0 = 1, NX-2
             x=x0+1
